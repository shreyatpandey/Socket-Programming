...
while (1) {
     ...
     printf("The Server A received the input <%s>\n",buf);
    if (n < 0) 
      { 
...
/* square calculation */
    ...
    double square_value = pow(convertedresult,2); 
     /*double square_value = (convertedresult)*(convertedresult);*/
     printf("The server A calculated square <%g>\n",square_value);  
     char buf_send[1024]; 
     snprintf(buf_send,sizeof(buf_send),"%.10f",square_value);
